...
public class PersistenceManager {
  ...
  public void treeToJson(String directory) {
    ...
    if (!directory.endsWith(".json")) {
      throw new IllegalArgumentException("directory does not end with '.json'");
    }
    ...
    for (TreeIterator it = this.root.createIterator().first(); it != null; it = it.next()) {
      ...
      if (container instanceof Task) {
        ...
        ((Task) container).getIntervals().forEach(interval -> {
          ...
          intervalJsonObj = jsonObjFromData("Interval", Integer.toString(interval.getId()),
              null, interval.getParent(), interval.getTotalTime(), interval.getInitialDate(),
      ...
      containerJsonObj = jsonObjFromData(containerType, Integer.toString(container.getId()),
          container.getName(), container.getParent(), container.getTotalTime(),
          container.getInitialDate(), container.getFinalDate());
  ...
  private JSONObject jsonObjFromData(String type, String id, String name,
                                     Container parent, Duration totalTime, LocalDateTime
                                         initialDate, LocalDateTime finalDate) {
    // Precondition
    assert ((type.equals("Project") || type.equals("Task")
        || type.equals("Interval")) && (totalTime != null));
  ...
  public void jsonToTree(String directory) throws IOException {
    ...
    try (FileInputStream is = new FileInputStream(directory)) {
      ...
      // Obtenemos los JsonArrays de contenedores e intervalos
      // a partir del JSON object
      JSONArray jsonContainers = object.getJSONArray("Containers");
      JSONArray jsonIntervals = object.getJSONArray("Intervals");

      // Iteramos sobre el jsonArray de contenedores
      // para obtener los datos de los proyectos y tareas
      for (int i = 0; i < jsonContainers.length(); i++) {
        ...
        // Obtenemos un string del tiempo total y lo transformamos
        // en un objeto de "Duration"
        ...
        // Obtenemos un string de la fecha inicial y final, y las transformamos
        // en objetos de "LocalDateTime"
        ...
        // Comprobamos si este contenedor es un proyecto o una tarea y creamos el contenedor.
        // tambien lo añadimos a los diccionarios previamente creados
      ...
      for (int i = 0; i < jsonIntervals.length(); i++) {
        ...
        // Creamos el intervalo y lo añadimos a la lista de
        // intervalos de la tarea a la que pertenece
